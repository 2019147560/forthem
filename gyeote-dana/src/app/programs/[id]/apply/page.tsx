...
export default function ApplyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [data, setData] = useState<FormData>({
    nick: '', age: '', region: '', motive: '', current: '', expect: '',
    pace: '', contact: '', mode: '', consent1: false, consent2: false,
  });
  ...
  return (
    <main className="shell">
      ...
      <Link href={`/programs/${p.id}`} className="back">
        <Icon name="chev-left" size={14} /> {p.title}
      </Link>
      ...
      <div className="apply-shell">
        ...
        {step === 0 && (
          <div className="form-card">
            ...
            <div className="form-foot">
              <span />
              <button className="btn btn-primary" disabled={!canNext()} onClick={() => setStep(1)}>다음 <Icon name="chev-right" size={14} /></button>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="form-card">
            ...
            <div className="form-foot">
              <button className="btn btn-ghost" onClick={() => setStep(0)}><Icon name="chev-left" size={14} /> 이전</button>
              <button className="btn btn-primary" disabled={!canNext()} onClick={() => setStep(2)}>다음 <Icon name="chev-right" size={14} /></button>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="form-card">
            ...
            <div className="field">
              ...
              <div className="choice-row">
                {['문자', '카카오톡', '이메일', '연락 안 하셔도 됩니다 (앱 내 알림만)'].map((v) => (
                  <button key={v} className={`choice${data.contact === v ? ' selected' : ''}`} onClick={() => set('contact', v)}>{v}</button>
                ))}
              </div>
            </div>
            <div className="form-foot">
              <button className="btn btn-ghost" onClick={() => setStep(1)}><Icon name="chev-left" size={14} /> 이전</button>
              <button className="btn btn-primary" disabled={!canNext()} onClick={() => setStep(3)}>다음 <Icon name="chev-right" size={14} /></button>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="form-card">
            ...
            <div style={{ background: 'var(--bg-soft)', borderRadius: 4, padding: 16, marginBottom: 20, border: '1px solid var(--line)' }}>
            ...
            <div className="form-foot">
              <button className="btn btn-ghost" onClick={() => setStep(2)}><Icon name="chev-left" size={14} /> 이전</button>
              <button className="btn btn-primary" disabled={!canNext()} onClick={() => setStep(4)}>제출하기</button>
            </div>
          </div>
        )}
        ...
        {step === 4 && (
          <div className="form-card confirm-card">
            ...
            <h2 className="form-h" style={{ fontSize: 24, border: 'none', padding: 0 }}>신청이 접수됐어요</h2>
            <p className="form-sub" style={{ fontSize: 14, maxWidth: 440, margin: '12px auto 28px' }}>
              담당 매니저가 영업일 기준 2~3일 내 연락 드릴게요.<br />
              마음의 부담 없이, 천천히 기다려 주세요.
            </p>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
              <Link href="/" className="btn btn-ghost" style={{ width: 'auto', padding: '0 22px' }}>홈으로</Link>
              <Link href="/my" className="btn btn-primary" style={{ width: 'auto', padding: '0 22px' }}>내 신청 보기</Link>
            </div>
          </div>
        )}
      </div>
